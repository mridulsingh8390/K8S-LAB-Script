...
## Optional variant: running the database in-cluster instead of managed RDS

This repo also includes a separate, optional path: running PostgreSQL as
a pod inside the cluster, with its data on Amazon EFS, instead of using
managed RDS SQL Server with a private subnet + security group. `run-all.sh`
does **not** touch any of this — it's entirely opt-in.

The relevant files:
```
00b-aws-efs.sh                  - EFS file system, mount targets, security group
01b-install-efs-csi-driver.sh   - EFS CSI driver (IRSA + Helm) + StorageClass
dev-postgres-efs/                - PVC, SecretProviderClass, Postgres StatefulSet,
                                    and an app Deployment pointed at Postgres
```

This is genuinely **simpler infrastructure than the AKS lab's equivalent
ANF/Trident variant** — the EFS CSI driver is AWS's own officially
maintained driver, installed via a normal Helm chart with IRSA (the same
identity pattern already used everywhere else in this lab), with no
third-party operator and no separate per-cloud service principal to
manage. That's a genuine, structural difference between the two clouds,
not just smaller YAML files.
...
string format in `dev-postgres-efs/03-app-deployment-postgres.yaml` makes
...
infrastructure piece (EFS access point provisioning, Postgres starting,
...
Brief sequence (assumes `00-aws-infra.sh` has already run, since this
reuses that VPC/EKS cluster):
```bash
./00b-aws-efs.sh                     # EFS file system, mount targets, security group
./01b-install-efs-csi-driver.sh      # CSI driver for dynamic EFS provisioning

# generate and store the Postgres password yourself (not automated,
# since Postgres isn't created via an AWS control-plane API call):
...
the main `cleanup.sh`, or these EFS-specific resources will be left
