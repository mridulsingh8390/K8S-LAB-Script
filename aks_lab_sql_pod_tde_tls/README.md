...
k8s/k8s/dev-postgres-anf/           - PVC, SecretProviderClass, Postgres StatefulSet,
...
string format in `k8s/k8s/dev-postgres-anf/03-app-deployment-postgres.yaml` makes
...
sed -i \
  ...
  k8s/k8s/dev-postgres-anf/01-secretproviderclass.yaml
sed -i "s|<ACR_NAME>|${ACR_NAME}|g" k8s/k8s/dev-postgres-anf/03-app-deployment-postgres.yaml

kubectl apply -f k8s/k8s/dev-postgres-anf/00-pvc.yaml
kubectl apply -f k8s/k8s/dev-postgres-anf/01-secretproviderclass.yaml
kubectl apply -f k8s/k8s/dev-postgres-anf/02-statefulset.yaml
...
kubectl apply -f k8s/k8s/dev-postgres-anf/03-app-deployment-postgres.yaml
```

Tearing this variant down (not covered by the main `cleanup.sh`):
```bash
kubectl delete -f k8s/k8s/dev-postgres-anf/
